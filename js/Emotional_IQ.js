...
function initialize() {
  ...
  userInputs = [];
  statements = ["are Homo Erectus Level (c'mon, you could do better)", "are an Absolute Moron (you're not much better than your ancient homo erectus ancestors)", "have an Empathetic Deficiency", "are Socially Inactive", "are Borderline Sociable", "are Normal", "are Just Above Average", "have Superior Emotional Intelligence", "have better Emotional Intelligence than most people around you", "are Gifted in Emotional Intelligence", "are Incredibly proficient (social scientist level)"];
}
...
function calculateResults() {
  iq = point * score; // calculate IQ

  if (userLastQuestion) {
    resultsOutput.innerHTML = "<h3>Your final score is " + score + " out of " +
      total + "</h3> <h4 id = 'statement'> Your Emotional IQ is " + iq + ", which means that you " + getEmotionalStatement() + "</h4> <a href='faceQuiz.html'> Would you like to Take This Quiz Again?</a >";
...
function getEmotionalStatement() {
  for (var i = 0; i <= 10; i++) {
    if (score == i) {
